(function (window) {
  ...
  // Builds that don't use the prediction service (e.g. the Chrome extension)
  // leave the DOLPHIN_URL placeholder unsubstituted. An unsubstituted value is
  // a *relative* URL, so any fetch against it would hit the tracked site's own
  // server and leak the token in the Authorization header — every
  // prediction-service network path must be gated on this flag.
  const DOLPHIN_ENABLED = /^https?:\/\//.test(DOLPHIN_URL);
  const LIB_VERSION = "1.0.22"; // Constant library version - cannot be changed by client
  // *** this version is changed, we added sg rules config which does not exist in 1.0.22 ver of tag
  ...
  // __MOVEO_SG_RULES_SANITIZE_BEGIN__ (run-tests.mjs extracts this block by
  // the markers and unit-tests it — keep it self-contained.)
  //
  // semanticGroupRules: config-driven sg grouping. Each rule declares a
  // `container` selector ("this element is one product/unit — everything
  // inside it belongs to it") and an ordered `name` source list resolved
  // against that container ("self" = the container itself, anything else =
  // container.querySelector; `attribute` absent = textContent). mode
  // "first" (default) takes the first non-empty source, "join" concatenates
  // all non-empty sources with "_". Structural sanitation happens here at
  // init; selector validity is only checkable against a live DOM, so it is
  // enforced at use time (try/catch in resolveSemanticGroupFromRules).
  const SG_RULES_MAX_RULES = 20;
  const SG_RULES_MAX_SOURCES = 5;
  const SG_RULES_MAX_SELECTOR_LEN = 250;
  const SG_RULES_MAX_ATTR_LEN = 100;
  // Rule-derived names keep more of the source text than the 50-char
  // heuristic cap: product names are the payload here, and "join" mode
  // prefixes them with an id.
  const SG_RULES_MAX_NAME_LEN = 80;

  function sanitizeSemanticGroupRules(raw) {
    if (!Array.isArray(raw)) return [];
    const out = [];
    for (const entry of raw) {
      if (out.length >= SG_RULES_MAX_RULES) break;
      if (entry === null || typeof entry !== "object" || Array.isArray(entry))
        continue;
      const container =
        typeof entry.container === "string" ? entry.container.trim() : "";
      if (!container || container.length > SG_RULES_MAX_SELECTOR_LEN) continue;
      if (!Array.isArray(entry.name)) continue;
      const name = [];
      for (const src of entry.name) {
        if (name.length >= SG_RULES_MAX_SOURCES) break;
        if (src === null || typeof src !== "object" || Array.isArray(src))
          continue;
        const source = typeof src.source === "string" ? src.source.trim() : "";
        if (!source || source.length > SG_RULES_MAX_SELECTOR_LEN) continue;
        const attribute =
          typeof src.attribute === "string" &&
          src.attribute.trim() &&
          src.attribute.trim().length <= SG_RULES_MAX_ATTR_LEN
            ? src.attribute.trim()
            : null;
        name.push({ source, attribute });
      }
      if (!name.length) continue;
      out.push({
        container,
        name,
        mode: entry.mode === "join" ? "join" : "first",
      });
    }
    return out;
  }
  // __MOVEO_SG_RULES_SANITIZE_END__

  // __MOVEO_TRACK_SELECTORS_SANITIZE_BEGIN__ (run-tests.mjs extracts this
  // block by the markers and unit-tests it — keep it self-contained.)
  //
  // additionalTrackedElements: config-driven force-tracking. Each entry's
  // `selector` marks elements that must be tracked (appear/click/hover)
  // even when the heuristics in shouldTrackElement would roll them up or
  // drop them; the optional `type` overrides those elements' event type
  // (eT) so lookalike tags (e.g. three price spans) stay distinguishable
  // in the data. Purely additive: existing tracked elements are unaffected.
  // Structural sanitation happens here; selector syntax needs a live DOM,
  // so it is enforced at compile time (compileForcedTrackSelector).
  const TRACK_SELECTORS_MAX_RULES = 20;
  const TRACK_SELECTORS_MAX_SELECTOR_LEN = 250;
  const TRACK_SELECTORS_MAX_TYPE_LEN = 50;

  function sanitizeAdditionalTrackedElements(raw) {
    if (!Array.isArray(raw)) return [];
    const out = [];
    for (const entry of raw) {
      if (out.length >= TRACK_SELECTORS_MAX_RULES) break;
      if (entry === null || typeof entry !== "object" || Array.isArray(entry))
        continue;
      const selector =
        typeof entry.selector === "string" ? entry.selector.trim() : "";
      if (!selector || selector.length > TRACK_SELECTORS_MAX_SELECTOR_LEN)
        continue;
      const type =
        typeof entry.type === "string" &&
        entry.type.trim() &&
        entry.type.trim().length <= TRACK_SELECTORS_MAX_TYPE_LEN
          ? entry.type.trim()
          : null;
      out.push({ selector, type });
    }
    return out;
  }
  // __MOVEO_TRACK_SELECTORS_SANITIZE_END__

  // Name/id/placeholder/aria-label fragments that indicate a sensitive field
  const SENSITIVE_NAME_REGEX =
    /(address|street|city|state|zip|postal|region|country|firstname|first[_-]?name|lastname|last[_-]?name|fullname|full[_-]?name|(^|[_-])name([_-]|$)|email|phone|tel|mobile|dob|birth|ssn|tax|vat|pib|jmbg|oib|iban|card|cc[_-]?num|cardnum|cvv|cvc|security[_-]?code|passport|license)/i;

  // WHATWG autofill tokens that indicate a sensitive field
  const SENSITIVE_AUTOCOMPLETE_REGEX =
    /^(street-address|address-line[1-3]|address-level[1-4]|postal-code|country|country-name|name|given-name|additional-name|family-name|honorific-prefix|honorific-suffix|nickname|organization|email|tel|tel-[a-z-]+|bday|bday-[a-z]+|cc-[a-z-]+|new-password|current-password|one-time-code)$/i;
  ...
  const SENSITIVE_CONTAINER_SELECTOR =
    'address, [itemtype*="PostalAddress"], .woocommerce-customer-details, .woocommerce-customer-details address, .customer_details, [class*="billing_address"], [class*="shipping_address"], [class*="customer-address"]';
  ...
  class MoveoOneWeb {
    constructor(token) {
      ...
      this.semanticGroupRules = []; // sanitized in init(); empty ⇒ heuristic sg only
      this.additionalTrackedElements = []; // sanitized in init(); selectors forced into tracking
      this.forcedTrackSelector = null; // comma-joined valid selectors; null ⇒ feature off
    ...
    getPlatformInfo() {
      try {
        ...
        if (navigator.appVersion) {
          ...
          if (appVersion.includes("iphone") || appVersion.includes("ipad"))
            return "iOS";
    ...
    initialize() {
      ...
      if (this.initialized) {
        if (LOGGING_ENABLED) {
          console.warn(
            "MoveoOne: Already initialized, ignoring duplicate initialize() call",
          );
        }
        return;
      }
    ...
    preconnectToPredictionService() {
      ...
      try {
        ...
        if (LOGGING_ENABLED) {
          console.log(
            "MoveoOne: DNS prefetch and preconnect established for prediction service:",
            preconnectUrl,
          );
        }
      } catch (error) {
      ...
      try {
        ...
        scheduleWarmup(async () => {
          try {
            ...
            if (LOGGING_ENABLED) {
              console.log(
                `MoveoOne: Warmup successful - ${response.status} in ${warmupTime.toFixed(1)}ms for ${healthEndpoint}`,
              );
            }
            ...
            if (LOGGING_ENABLED) {
              console.warn(
                `MoveoOne: Warmup failed after ${warmupTime.toFixed(1)}ms:`,
                error.message,
              );
            }
        ...
        if (LOGGING_ENABLED) {
          console.log(
            "MoveoOne: Prediction service warmup scheduled (zero performance impact)",
          );
        }
      } catch (error) {
    ...
    sendAutoImpressionWithTimestamp(el, rect, action, timestamp) {
      if (
        this.exclude_detailed_tracking &&
        (action === "appear" || action === "disappear")
      )
        return;
    ...
    _scheduleUserDataSync(delayMs) {
      ...
      this._userDataSyncTimer = setTimeout(() => {
        ...
        this.syncUserDataFromStorage();
        const allKeysPresent = this.userDataKeys.every(
          (k) => this.meta[k] != null,
        );
        if (allKeysPresent || this._userDataSyncAttempts >= MAX_ATTEMPTS)
          return;
    ...
    async start() {
      ...
      if (this.started) {
        if (LOGGING_ENABLED) {
          console.warn(
            "MoveoOne: Session already started, ignoring duplicate start() call",
          );
        }
        return;
      }
    ...
    getElementFullText(element) {
      ...
      try {
        ...
        // Force-tracked elements are treated as one semantic unit, like the
        // text containers above: full subtree text, not just direct nodes
        // (a configured wrapper span would otherwise report an empty value).
        if (this.isForcedTrackTarget(element)) {
          let text = element.textContent || element.innerText || "";
    ...
    getSemanticGroup(element) {
      ...
      // Highest priority: config-driven container rules. They outrank the
      // data-moveo-element-id channel because rules are authored per study
      // for exactly this page, while the data attrs are generic site
      // instrumentation the study author may need to override.
      const ruleSg = this.resolveSemanticGroupFromRules(element);
      if (ruleSg != null) {
        return ruleSg;
      }
    ...
    cleanSemanticGroupName(name, maxLen = 50) {
    ...
    // Config-driven sg resolution — the highest-priority branch of
    // getSemanticGroup. Walks the sanitized semanticGroupRules in order:
    // the first rule whose `container` is an ancestor-or-self of the event
    // element wins, and its name is resolved from the rule's `name` sources
    // (see sanitizeSemanticGroupRules for the shape). Returns null on any
    // miss so the caller falls through to the heuristic naming — a bad
    // selector or missing node can only ever degrade to today's behavior,
    // never break the host page.
    resolveSemanticGroupFromRules(element) {
      const rules = this.semanticGroupRules;
      ...
      for (const rule of rules) {
        let container = null;
        try {
          container = element.closest(rule.container);
        } catch (error) {
          continue; // invalid selector from config
        }
        if (!container) continue;

        const parts = [];
        for (const src of rule.name) {
          let node = null;
          try {
            node =
              src.source === "self"
                ? container
                : container.querySelector(src.source);
          } catch (error) {
            continue; // invalid selector from config
          }
          if (!node) continue;
          ...
          try {
            raw = src.attribute
              ? node.getAttribute(src.attribute)
              : node.textContent;
          } catch (error) {
            continue;
          }
          ...
          if (!raw) continue;
          parts.push(raw);
          if (rule.mode !== "join") break; // "first": one non-empty source is enough
        }
        if (!parts.length) continue;

        const cleaned = this.cleanSemanticGroupName(
          parts.join("_"),
          SG_RULES_MAX_NAME_LEN,
        );
        if (cleaned && cleaned !== "global") return cleaned;
      }
      return null;
    }
    ...
    // Config-driven force-tracking (additionalTrackedElements). Both helpers
    // are no-ops (null selector) when the feature is unconfigured.
    isForcedTrackTarget(el) {
      if (!this.forcedTrackSelector) return false;
      if (!el || el.nodeType !== 1 || typeof el.matches !== "function")
        return false;
      try {
        return el.matches(this.forcedTrackSelector);
      } catch (e) {
        return false;
      }
    }

    // Nearest ancestor-or-self matching a forced selector — used only to
    // rescue clicks/hovers that today's pipeline would drop entirely.
    findForcedTrackAncestor(el) {
      if (!this.forcedTrackSelector) return null;
      if (!el || el.nodeType !== 1 || typeof el.closest !== "function")
        return null;
      try {
        return el.closest(this.forcedTrackSelector);
      } catch (e) {
        return null;
      }
    }

    // Validate each configured selector against a detached probe element
    // (invalid syntax throws) and pre-join the survivors so the hot path
    // pays a single matches() call. Fail-open per selector.
    compileForcedTrackSelector() {
      if (!this.additionalTrackedElements.length) return null;
      const probe = document.createElement("i");
      const valid = this.additionalTrackedElements
    ...
    // Config-provided event type (eT) for a force-tracked element: the first
    // matching entry that sets `type` provides it. Null when unconfigured or
    // no typed entry matches — callers then fall back to normal eT rules.
    getForcedTrackType(el) {
      if (!this.forcedTrackSelector) return null;
      if (!this.isForcedTrackTarget(el)) return null;
      for (const rule of this.additionalTrackedElements) {
        if (!rule.type) continue;
        try {
          if (el.matches(rule.selector)) {
            const cleaned = this.cleanSemanticGroupName(rule.type);
            if (cleaned && cleaned !== "global") return cleaned;
          }
        } catch (e) {
          // invalid selector — skip, fail-open
        }
      }
      return null;
    }
    ...
    getMoveoEventType(el, fallbackType) {
      // Config type for force-tracked elements outranks the site's
      // data-moveo attribute (study config wins, §3.2) — and every event
      // kind (appear/disappear/click/hover) computes eT through here.
      const forcedType = this.getForcedTrackType(el);
    ...
    shouldTrackElement(element) {
      ...
      // Config-driven force-tracking: same contract as the explicit
      // data-moveo-element-track branch above (and it outranks that
      // attribute's "false" opt-out — study config wins). Skips the
      // bounds/text/roll-up heuristics below by design.
      if (this.isForcedTrackTarget(element)) {
        if (element.id && element.id.includes("moveo")) return false;
        if (element.className && String(element.className).includes("moveo")) {
          return false;
      ...
      if (
        this.isSmallTextElement(element) &&
        this.hasTextContainerParent(element)
      ) {
        return false;
      }
      ...
      if (
        this.isTextContainerElement(element) &&
        this.hasTextContainerParent(element)
      ) {
        return false;
      }
    ...
    setupHoverTracking() {
      ...
      document.addEventListener("mouseover", (event) => {
        ...
        if (!this.shouldTrackElement(target)) {
          // Force-track rescue: the pointer is usually over a child of a
          // configured element. Only hovers that would be dropped today are
          // retargeted, so existing hover behavior is unchanged.
          const forced = this.findForcedTrackAncestor(target);
          if (!forced || !this.shouldTrackElement(forced)) return;
          target = forced;
        }
    ...
    setupClickTracking() {
      ...
      const handleClick = (event) => {
        ...
        let interactiveElement = findInteractiveParent(target);

        // Check if we should track this element
        if (!this.shouldTrackElement(interactiveElement)) {
          // Force-track rescue: retarget only clicks the pipeline would drop
          // entirely. A click that emits an event today (button, link, ...)
          // never reaches this branch, so its attribution cannot change.
          const forced = this.findForcedTrackAncestor(target);
          if (forced && this.shouldTrackElement(forced)) {
            interactiveElement = forced;
          } else {
            ...
            if (LOGGING_ENABLED) {
              console.log("MoveoOne: Element not tracked:", {
                ...
                visible:
                  interactiveElement.getBoundingClientRect().width > 0 &&
                  interactiveElement.getBoundingClientRect().height > 0,
              });
            }
            return;
          }
    ...
    getNestedScrollContainerScore(element) {
      ...
      if (
        element.clientHeight < minClientHeight ||
        element.clientWidth < minClientWidth
      ) {
        return null;
      }
    ...
    isSensitiveField(el) {
      ...
      if (
        SENSITIVE_NAME_REGEX.test(name) ||
      ...
      try {
        if (id && document.querySelector) {
          ...
          if (lbl && SENSITIVE_NAME_REGEX.test((lbl.textContent || "").trim()))
            return true;
        }
      } catch (_) {
      ...
      while (anc) {
        if (anc.tagName === "LABEL") {
          if (SENSITIVE_NAME_REGEX.test((anc.textContent || "").trim()))
            return true;
          break;
        }
    ...
    async internalPredict(modelId, sessionId) {
      if (!DOLPHIN_ENABLED) {
        return {
          success: false,
          status: "disabled",
          message: "Prediction service is not available in this build",
        };
      }
      ...
      try {
        ...
        if (response.status === 423) {
          ...
          return {
            ...
            status: "pending",
            message:
              responseData.detail || "Model is loading, please try again",
          };
        }
        ...
        if (response.status === 412) {
          ...
          return {
            ...
            message:
              errorData.detail ||
              "Prediction skipped due to A/B test configuration",
          };
        }
        ...
        if (response.status === 422) {
          ...
          if (
            errorData.detail &&
            errorData.detail.includes("Completion target already reached")
          ) {
            return {
              success: false,
  ...
  window.MoveoOne = {
    init: function (token, options = {}) {
      ...
      if (window.MoveoOne.instance) {
        if (LOGGING_ENABLED) {
          console.warn(
            "MoveoOne: Already initialized, returning existing instance",
          );
        }
      ...
      instance.type = allowedTypes.includes(requestedType)
        ? requestedType
        : "STATIC_WEBSITE";
      ...
      instance.storageSource = allowedStorageSources.includes(requestedStorage)
        ? requestedStorage
        : "local";
      ...
      const rawKeys = Array.isArray(options.userDataKeys)
        ? options.userDataKeys
        : [];
      instance.userDataKeys = rawKeys.filter(
      ...
      // Config-driven semantic-group rules (container grouping + naming).
      // Malformed entries are dropped silently; an empty result simply
      // leaves the heuristic sg behavior untouched.
      instance.semanticGroupRules = sanitizeSemanticGroupRules(
        options.semanticGroupRules,
      );

      // Config-driven force-tracking. Compiled before initialize() runs, so
      // the initial DOM sweep already sees the selector; empty/invalid input
      // leaves forcedTrackSelector null and every hot path short-circuits.
      instance.additionalTrackedElements = sanitizeAdditionalTrackedElements(
        options.additionalTrackedElements,
      );
      instance.forcedTrackSelector = instance.compileForcedTrackSelector();
      ...
      Object.keys(options).forEach((key) => {
        if (
          ...
          key === "storageSource" ||
          key === "userDataKeys" ||
          key === "exclude_detailed_tracking" ||
          key === "semanticGroupRules" ||
          key === "additionalTrackedElements"
        )
    ...
    predict: function (modelId) {
      ...
      if (!window.MoveoOne.instance) {
        return Promise.resolve({
          ...
          message:
            "MoveoOne must be initialized before using predict method. Call MoveoOne.init() first.",
        });
      }
      ...
      if (!sessionId) {
        return Promise.resolve({
          ...
          status: "no_session",
          message:
            "No active session found. Please ensure MoveoOne is properly initialized.",
        });
      }

      // Call the internal predict method asynchronously
      // This is non-blocking and won't affect website performance
      return window.MoveoOne.instance.internalPredict(
        modelId.trim(),
        sessionId,
      );
    },
